...
function Statistics() {
    const { owners, teams, statistics } = useContext(SupabaseContext);
                ...
                <div className="flex flex-row gap-5 w-[90%]">
                    {
                        tabs.map((element, index) => (
                            <TabButton className={index === activeIndex ? "bg-red-100 text-red-500 border-solid border-2 border-red-100" : "bg-white text-gray-400 border-solid border-2 border-gray-100"} key={element} text={element} onClick={() => setIndexAndSelect(index)}/>
                        ))
                    }
                </div>

                <div className="flex flex-row items-center gap-3 w-[90%]">
                    <label className="font-semibold text-xs text-gray-400" htmlFor="sortBy">Sort by:</label>
                    <select className="bg-white text-gray-400 border-solid border-2 border-gray-100 font-semibold px-4 py-2 rounded-2xl text-xs w-48" id="sortBy" value={sortBy} onChange={handleChange}>
                        {
